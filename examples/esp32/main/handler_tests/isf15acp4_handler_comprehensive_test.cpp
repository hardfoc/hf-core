...
extern "C" void app_main(void) {
    ...
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_CONSTRUCTION_TESTS, "CONSTRUCTION",
        RUN_TEST("construct", test_construction(), &g_test_results);
    );
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_INITIALIZATION_TESTS, "INITIALIZATION",
        RUN_TEST("initialize", test_initialize(), &g_test_results);
        RUN_TEST("is_initialized", test_is_initialized(), &g_test_results);
    );
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_DISPLAY_TESTS, "DISPLAY",
        RUN_TEST("display_on_off", test_display_on_off(), &g_test_results);
        RUN_TEST("fill_screen", test_fill_and_present(), &g_test_results);
    );
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_GRAPHICS_TESTS, "GRAPHICS",
        RUN_TEST("graphics_context", test_graphics_context(), &g_test_results);
    );
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_BUTTON_TESTS, "BUTTON",
        RUN_TEST("button_ui", test_button_ui(), &g_test_results);
    );
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_DRIVER_ACCESS_TESTS, "DRIVER ACCESS",
        RUN_TEST("visit_driver", test_visit_driver(), &g_test_results);
        RUN_TEST("get_driver", test_get_driver(), &g_test_results);
    );
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_DIAGNOSTICS_TESTS, "DIAGNOSTICS",
        RUN_TEST("error_flags", test_error_flags(), &g_test_results);
        RUN_TEST("dump_diag", test_dump_diagnostics(), &g_test_results);
    );
    RUN_TEST_SECTION_IF_ENABLED(ENABLE_ERROR_HANDLING_TESTS, "ERROR HANDLING",
        RUN_TEST("before_init", test_operations_before_init(), &g_test_results);
        RUN_TEST("deinitialize", test_deinitialize(), &g_test_results);
    );
