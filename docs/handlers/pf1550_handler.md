...
pmic.ApplyProfile(host_register_table);
```

`ApplyPortentaH7*` methods are driver-named eval sequences (Arduino Portenta
H7). Hosts that are not that board pass their own table to `ApplyProfile`.
