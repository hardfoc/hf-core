...
void HalIsf15acp4SpiAdapter::setGpio(BaseGpio& gpio, bool active) noexcept {
    gpio.SetPinLevel(active ? hf_gpio_level_t::HF_GPIO_LEVEL_HIGH
                            : hf_gpio_level_t::HF_GPIO_LEVEL_LOW);
}

bool HalIsf15acp4SpiAdapter::readGpio(BaseGpio& gpio, bool active_low) noexcept {
    hf_gpio_level_t level = hf_gpio_level_t::HF_GPIO_LEVEL_LOW;
    if (gpio.GetPinLevel(level) != hf_gpio_err_t::GPIO_SUCCESS) {
        return false;
    }
    const bool high = level == hf_gpio_level_t::HF_GPIO_LEVEL_HIGH;
